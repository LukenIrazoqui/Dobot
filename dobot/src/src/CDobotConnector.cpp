...
void CDobotConnector::connectDobot(void *isFinishedAddr, void *resultAddr, void *portNameAddr, unsigned int baudrate, void *fwTypeAddr, void *versionAddr, void *timeAddr)
{
    bool *isFinished = (bool *)isFinishedAddr;
    int *result = (int *)resultAddr;
    char *fwType = (char *)fwTypeAddr;
    char *version = (char *)versionAddr;
    float *time = (float *)timeAddr;
    ...
    if (!regExpIP.match(connectPortName).hasMatch()) {
    ...
    if (fwType && strcmp(fwType, "DobotWifi") != 0) {
        QRegularExpression regExpGRBL("GRBL\\:\\sV((\\d+\\.){2}\\d+)");
        QRegularExpression regExpMARLIN("MARLIN\\:\\sV((\\d+\\.){2}\\d+)");
        QRegularExpression regExpMARLINTime("Runtim1:-?(\\d*.\\d*),");
        ...
        while(true) {
            ...
            if(ioDevice->read(recvData, recvSize) > 0) {
                ...
                QRegularExpressionMatch matchGRBL = regExpGRBL.match(recvStr);
                if(matchGRBL.hasMatch()) {
                    if (fwType) {
                        strcpy(fwType, "GRBL");
                    }
                    if (version) {
                        strcpy(version, matchGRBL.captured(1).toLatin1().data());
                    }
                    ioDevice->close();
                    ioDevice->deleteLater();
                    ioDevice = NULL;
                ...
                QRegularExpressionMatch matchMARLIN = regExpMARLIN.match(recvStr);
                if(matchMARLIN.hasMatch()){
                    if (fwType) {
                        strcpy(fwType, "MARLIN");
                    }
                    if (version) {
                        strcpy(version, matchMARLIN.captured(1).toLatin1().data());
                    }
                    QRegularExpressionMatch matchMARLINTime = regExpMARLINTime.match(recvStr);
                    if (matchMARLINTime.hasMatch()) {
                        if (time) {
                           *time = static_cast<float>(matchMARLINTime.captured(1).toDouble());
                        }
                        ioDevice->close();
                        ioDevice->deleteLater();
                        ioDevice = NULL;
...
void CDobotConnector::getMarlinVersion(void *isFinishedAddr, void *resultAddr)
{
    bool *isFinished = (bool *)isFinishedAddr;
    int *result = (int *)resultAddr;
